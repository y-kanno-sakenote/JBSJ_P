...
def summary_global_filters(y_from: int, y_to: int, genre_sel: list[str] | None, l1_sel: list[str] | None, l2_sel: list[str] | None) -> str:
...
def render_provenance_banner_from_df(df_use: pd.DataFrame, total_n: int, y_from: int | None = None, y_to: int | None = None, genre_sel: list[str] | None = None, l1_sel: list[str] | None = None, l2_sel: list[str] | None = None) -> None:
    try:
        ...
        l1_sel = [t for t in (l1_sel or []) if str(t).strip()]
        l2_sel = [t for t in (l2_sel or []) if str(t).strip()]
        ...
        tg_txt = _get_txt("研究分野L1", l1_sel)
        tp_txt = _get_txt("専門領域L2", l2_sel)
...
def adapt_filter_bar_for_obj(df: pd.DataFrame):
    ...
    if isinstance(res, dict):
        ...
        l1_sel = list(res.get("l1", []) or res.get("targets", []) or [])
        l2_sel = list(res.get("l2", []) or res.get("types", [])   or [])
        df_use = apply_filters(df, int(y_from), int(y_to), genre_sel, l1_sel, l2_sel)
        return df_use, int(y_from), int(y_to), genre_sel, l1_sel, l2_sel
...
def augment_with_session_state(y_from: int, y_to: int, genre_sel: list[str], l1_sel: list[str], l2_sel: list[str], key_prefix: str = "obj"):
    try:
        ss = st.session_state
        ...
        if not l1_sel:
            l1_sel = _pick_list(f"{key_prefix}_l1", f"{key_prefix}_targets", f"{key_prefix}_tg")
        if not l2_sel:
            l2_sel = _pick_list(f"{key_prefix}_l2", f"{key_prefix}_types", f"{key_prefix}_tp")
        return int(y_from), int(y_to), genre_sel, l1_sel, l2_sel
    except Exception:
        return y_from, y_to, genre_sel, l1_sel, l2_sel